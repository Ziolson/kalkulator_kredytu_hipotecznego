...
## 2026-02-17 - Missing Security Headers in Nginx Configuration
**Vulnerability:** The `nginx.conf` file lacked critical security headers (HSTS, CSP, X-Frame-Options, etc.), leaving the application vulnerable to clickjacking, MIME-sniffing, and XSS attacks.
**Learning:** Default Nginx configurations often prioritize functionality over security. Even a static frontend app needs explicit security headers to protect users.
**Prevention:** Always audit and harden web server configurations (Nginx, Apache) by adding industry-standard security headers and a strict Content Security Policy.
