...
const ResultsDashboard: React.FC<Props> = ({
  ...
  const getPolishForm = useCallback(
    (count: number, forms: [string, string, string]) => {
      ...
      if (rem10 >= 2 && rem10 <= 4 && !(rem100 >= 12 && rem100 <= 14))
        return forms[1]; // np. lata, miesiące
      ...
      <div className="grid grid-cols-1 md:grid-cols-8 gap-4">
        ...
        <div className="md:col-span-5 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-3xl p-6 text-white shadow-xl shadow-emerald-900/10 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 transform translate-x-4 -translate-y-4">
            <span className="material-symbols-rounded text-[140px]">
              savings
            </span>
          </div>
          <div className="relative z-10 flex flex-col justify-between h-full">
            <div>
              <h2 className="text-emerald-100 font-medium text-xs uppercase tracking-wider mb-1">
                Twoje oszczędności
              </h2>
              ...
              <p className="text-emerald-100/80 text-sm font-medium">
                Tyle zostanie w Twojej kieszeni dzięki nadpłatom.
              </p>
            </div>
            <div className="mt-6 flex items-center gap-2">
              <span className="bg-white/20 backdrop-blur-md rounded-lg px-3 py-1 text-xs font-semibold text-white border border-white/20">
                Stopa zwrotu{" "}
                {results.totalCostStandard > 0
                  ? (
                      (totalMoneySaved / results.totalCostStandard) *
                      100
                    ).toFixed(0)
                  : 0}
                %
        ...
        <div className="md:col-span-3 bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-100 dark:border-slate-700 shadow-lg shadow-slate-200/50 dark:shadow-slate-950/50 flex flex-col justify-between relative overflow-hidden transition-colors duration-300">
          ...
          <div className="relative z-10 mt-4 pt-4 border-t border-slate-50 dark:border-slate-700 flex items-center gap-2">
            <span
              className={`material-symbols-rounded text-xl ${monthsSaved > 0 ? "text-emerald-500 dark:text-emerald-400" : "text-slate-400 dark:text-slate-500"}`}
            >
              {monthsSaved > 0 ? "history_toggle_off" : "event"}
            </span>
            <span
              className={`font-bold text-sm whitespace-nowrap ${monthsSaved > 0 ? "text-emerald-700 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}
            >
              {monthsSaved > 0
                ? `Szybciej o ${savedTimeText}`
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-xl shadow-slate-200/40 dark:shadow-slate-950/40 border border-slate-100 dark:border-slate-700 overflow-hidden transition-colors duration-300">
        ...
        <div className="flex border-b border-slate-100 dark:border-slate-700">
          ...
          >
            Wizualizacja
            {activeTab === "charts" && (
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-500 rounded-t-full"></div>
            )}
          </button>
          <button
            ...
            <div className="space-y-12 animate-in fade-in duration-300">
              ...
              <div>
                <div className="mb-6">
                  ...
                  <p className="text-sm text-slate-500">
                    Jak szybko znika Twój dług w porównaniu do standardowej
                    spłaty.
                  </p>
                </div>
              ...
              <div className="border-t border-slate-100 pt-10">
                <div className="mb-6">
                  ...
                  <p className="text-sm text-slate-500">
                    Zobacz jak nadpłata (zielony) zmniejsza udział odsetek
                    (czerwony) w czasie.
                  </p>
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 border-t border-slate-100 pt-10">
                <div>
                  <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-2">
                    Struktura rat (Rocznie)
                  </h3>
                  <p className="text-sm text-slate-500 mb-6">
                    Suma płatności w podziale na kapitał i odsetki.
                  </p>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 mb-6">
                    Całkowita kwota do oddania bankowi.
                  </p>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-50/50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
                    <tr>
                      ...
                      <th className="px-4 py-3 font-semibold text-right text-red-500">
                        Odsetki
                      </th>
                      <th className="px-4 py-3 font-semibold text-right text-blue-600">
                        Kapitał
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {yearlySchedule.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right font-medium text-red-500 dark:text-red-400">
                  ...
                  <tfoot className="bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-700">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-right font-bold text-slate-800 dark:text-slate-100">
                        {formatCurrency(results.totalCostStrategy)}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-red-600 dark:text-red-400">
                        {formatCurrency(results.totalInterestStrategy)}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-blue-700 dark:text-blue-400">
                        {formatCurrency(results.totalPrincipalStrategy)}
                      </td>
